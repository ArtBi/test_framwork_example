...
public abstract class BaseApiService {
    ...
    protected RequestSpecification setup() {
        return RestAssured.given().contentType(ContentType.JSON).accept(ContentType.JSON).filters(getFilters());
    }
    ...
    private List<Filter> getFilters() {
        ...
        if (enableLogging) {
            ...
            return List.of(new AllureRestAssured(), new RequestLoggingFilter(), new ResponseLoggingFilter());
        } else {
    ...
    protected AssertableResponse get(String endpoint, Map<String, String> pathParams, Map<String, String> queryParams) {
        log.debug("Executing GET request to endpoint: {} with pathParams: {} and queryParams: {}", endpoint, pathParams,
                queryParams);
        Response response = setup().pathParams(pathParams).queryParams(queryParams).when().get(endpoint);
        log.debug("Received response with status code: {}", response.getStatusCode());
        return new AssertableResponse(response);
    }
    ...
    protected AssertableResponse post(String endpoint, Map<String, String> pathParams,
                                      Map<String, String> queryParams) {
        log.debug("Executing POST request to endpoint: {} with pathParams: {} and queryParams: {}", endpoint,
                pathParams, queryParams);
        Response response = setup().pathParams(pathParams).queryParams(queryParams).when().post(endpoint);
        log.debug("Received response with status code: {}", response.getStatusCode());
        return new AssertableResponse(response);
    }
