package com.github.artbi.api.template.containers;

import com.github.artbi.api.template.config.PetstoreConfig;
...
/**
 * Container for the Swagger Petstore API. This container runs the Swagger Petstore Docker image and
 * exposes port 8080.
 */
@Slf4j
public class PetstoreContainer extends GenericContainer<PetstoreContainer> {

    private static final DockerImageName PETSTORE_IMAGE =
            DockerImageName.parse("swaggerapi/petstore3:latest");
    ...
    public PetstoreContainer() {
        ...
        withReuse(PetstoreConfig.get().reuseContainersEnabled());
        log.info("Configuring container to wait for HTTP endpoint: /api/v3/openapi.json");
        waitingFor(
                Wait.forHttp("/api/v3/openapi.json")
                        .forStatusCode(200)
                        .withStartupTimeout(Duration.ofMinutes(2)));
    }

    /**
     * Gets a singleton instance of the Petstore container. Thread-safe implementation using
